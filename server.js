...
function broadcastRooms(){
  const roomList = Object.keys(rooms).map(id=>({id, players: rooms[id].players.length}));
...
io.on("connection", socket=>{
  ...
  socket.on("makeMove", ({roomId, from, to})=>{
    ...
    if(!piece || getPlayer(piece)!==room.currentPlayer) return;
    room.board[to.r][to.c] = piece; room.board[from.r][from.c]="H";
    rekCapture(room.board,to.r,to.c);
    trappingCapture(room.board);
    const winner = checkWin(room.board);
    if(winner){ room.gameOver=true; io.to(roomId).emit("gameOver",winner); return; }
...
function rekCapture(board,r,c){
  const piece=board[r][c]; if(!piece||piece==="H") return;
  const player=getPlayer(piece), enemy=player==="red"?"blue":"red";
...
function trappingCapture(board){
  ...
  for(let r=0;r<8;r++){
    for(let c=0;c<8;c++){
      ...
      const player=getPlayer(board[r][c]), group=[], queue=[[r,c]];
      let hasEscape=false;
      visited[r][c]=true;
      while(queue.length){
        const [x,y]=queue.shift(); group.push([x,y]);
        [[0,1],[1,0],[0,-1],[-1,0]].forEach(([dx,dy])=>{
